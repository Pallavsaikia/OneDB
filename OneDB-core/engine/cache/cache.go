...
func (c *Cache) Set(key string, value interface{}, ttlS time.Duration) {
	c.mutex.Lock()
	defer c.mutex.Unlock()

	ttlDuration := time.Duration(ttlS) * time.Second

	// Check if the key already exists in the cache
	if elem, found := c.items[key]; found {
		// If the key exists, update its value and move it to the front of the eviction list
		elem.Value.(*CacheItem).Value = value
		elem.Value.(*CacheItem).ExpiryTime = time.Now().Add(ttlDuration)
